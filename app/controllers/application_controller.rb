class ApplicationController < ActionController::Base
    ...
    def define_title_list(mode)
        title_list = {"user" => ["score1","score2"],"route" => ["score1","score2"]}
        return title_list[mode]
    end
    ...
    def get_hash_main (data,title_list)
        ...
        title_list.each do |t|
            if num < score_list.length then
                score_num_hash[t] = score_list.to_i
            else
                score_num_hash[t] = 0            
            end
        end
    ...
    def get_user_score_hash num
        user = User.find(num)
        return get_hash_main( user.score, define_title_list("user"))
    end

    def get_route_score_hash num
        route = Route.find(num)
        return get_hash_main( route.score, define_title_list("route"))
    end
    ...
    def save_user_score_hash (num,hash)
        user = User.find(num)
        user.score = save_hash_main(hash, define_title_list("user"))
        user.save
    end

    def save_route_score_hash (num,hash)
        route = Route.find(num)
        route.score = save_hash_main(hash, define_title_list("route"))
        route.save
    end
